            ...
            del: function () {
            	...
            	layer.confirm('确定删除？', {
            		...
            		btn1 : function(index, layero) {
        				...
        				data['id'] = id;
                    	common.showLoading();
                        api.load('./color/delete','post',data, function(result) {
                            if (result.errcode == 0) {
                            	color.splice(deleteIndex);
...
function loadPageData(pageno) {
	common.showLoading();
	api.load(basePath + 'data/list','post',{"table":"color","pageno":pageno},function (result) {
		let pageView = result.data;
		setData(pageView);
		common.closeLoading();
	});
...
function showEditBox(idx,data) {
    ...
    layer.open({
        ...
        btn1: function (index, layero) {
            ...
            common.showLoading();
            api.load('./color/change','post',data, function(result) {
                if (result.errcode == 0) {
                	data = result.data;
                	if (idx < 0) {
                		color.push(data);
