            ...
            del: function () {
            	...
            	layer.confirm('确定删除？', {
            		...
            		btn1 : function(index, layero) {
        				...
        				data['id'] = id;
                    	common.showLoading();
                        api.load('./goodsgrouping/delete','post',data, function(result) {
                            if (result.errcode == 0) {
                            	goodsgrouping.splice(deleteIndex);
...
function loadData() {
    common.showLoading();
    api.load(basePath + 'data/list','post',{"table":"goodsgrouping"},function (result) {
        if (result.errcode == 0) {
...
function loadPageData(pageno) {
	common.showLoading();
	api.load(basePath + 'data/list','post',{"table":"goodsgrouping","pageno":pageno},function (result) {
		let pageView = result.data;
		setData(pageView);
		common.closeLoading();
	});
...
function showEditBox(idx,data) {
    ...
    layer.open({
        ...
        btn1: function (index, layero) {
            ...
            common.showLoading();
            api.load('./goodsgrouping/change','post',data, function(result) {
                if (result.errcode == 0) {
                	data = result.data;
                	if (idx < 0) {
                		goodsgrouping.push(data);
					} else {
						for (const key in data) {
	                        goodsgrouping[idx][key] = data[key];
	                    }
					}
